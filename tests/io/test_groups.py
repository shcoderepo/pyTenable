...
@pytest.mark.vcr()
def test_groups_list_success(api, group):
    res = api.groups.list()
    assert isinstance(res, list)
    for i in range(len(res)):
        if not res[i].get('immutable') or not res[i].get('membership_fixed') or not res[i].get('role'):
            check(res[i], 'name', str, allow_none=True)
            check(res[i], 'uuid', str, allow_none=True)
            check(res[i], 'user_count', int, allow_none=True)
            check(res[i], 'id', int, allow_none=True)
            check(res[i], 'container_uuid', str, allow_none=True)
        else:
            check(res[i], 'name', str, allow_none=True)
            check(res[i], 'uuid', str, allow_none=True)
            check(res[i], 'user_count', int, allow_none=True)
            check(res[i], 'id', int, allow_none=True)
            check(res[i], 'container_uuid', str, allow_none=True)
            check(res[i], 'role', str, allow_none=True)
            check(res[i], 'immutable', bool, allow_none=True)
            check(res[i], 'membership_fixed', bool, allow_none=True)
