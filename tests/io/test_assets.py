...
@pytest.mark.vcr()
def test_assets_details_success(api):
    """
    ...
    for j in resp['tags']:
        check(j, 'tag_uuid', 'uuid')
        check(j, 'tag_key', str)
        check(j, 'tag_value', str)
        check(j, 'added_by', 'uuid')
        check(j, 'added_at', 'datetime')

    check(resp, 'acr_score', str, allow_none=True)
    check(resp, 'exposure_score', str, allow_none=True)
    check(resp, 'acr_drivers', str, allow_none=True)
    check(resp, 'scan_frequency', int, allow_none=True)
    check(resp, 'interfaces', list)

    for k in resp['interfaces']:
        check(k, 'name', str)
        check(k, 'fqdn', list)
        check(k, 'mac_address', list)
        check(k, 'ipv4', list)
        check(k, 'ipv6', list)
    ...
    check(resp, 'exposure_confidence_value', str, allow_none=True)
