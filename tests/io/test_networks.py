...
@pytest.mark.vcr()
def test_networks_assign_scanners_success(api, network, scanner):
...
@pytest.mark.vcr()
def test_networks_network_asset_count(api, network):
    resp = api.networks.network_asset_count(network['uuid'], num_days=5)
    assert isinstance(resp, dict)
    check(resp, 'numAssetsTotal', int)
    check(resp, 'numAssetsNotSeen', int)

@pytest.mark.vcr()
def test_networks_calueerror(api, network):
    with pytest.raises(UnexpectedValueError):
